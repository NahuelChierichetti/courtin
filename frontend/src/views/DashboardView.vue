  ...
  <div class="space-y-6">
    ...
    <div class="flex items-start justify-between">
      ...
      <div class="flex items-center gap-3">
        ...
        <Button label="Exportar" icon="pi pi-download" severity="secondary" class="cursor-pointer !bg-white !border !border-slate-300 !text-slate-400" size="small" />
      </div>
    </div>
      ...
      <div class="col-span-2 rounded-2xl border border-slate-200 bg-white">
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div>
            ...
            <p class="text-xs text-slate-400">Confirmadas para hoy desde las {{ new Date().getHours() }}:{{ String(new Date().getMinutes()).padStart(2, '0') }} hs</p>
          </div>
          <button class="flex items-center gap-1 text-sm font-medium text-primitive-orange-500 hover:text-primitive-orange-600">
            Ver todas
          </button>
        </div>

        <div class="px-6">
          <div class="grid grid-cols-[80px_1fr_120px_100px_32px] items-center gap-6 border-b border-slate-100 py-3 text-xs font-semibold tracking-wide text-slate-400 uppercase">
            <span>Hora</span>
            <span>Cliente</span>
            <span>Cancha</span>
            <span>Pago</span>
          ...
          <div
            v-for="(reservation, index) in upcomingReservations"
            :key="index"
            class="grid grid-cols-[80px_1fr_120px_100px_32px] items-center gap-6 border-b border-slate-50 py-4 last:border-b-0"
          >
            ...
            <div>
              <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium font-secondary" :class="reservation.paymentPaid ? 'bg-success-50 text-success-500' : 'bg-red-50 text-red-500'">
                <span class="h-1.5 w-1.5 rounded-full" :class="reservation.paymentPaid ? 'bg-success-500' : 'bg-red-500'"></span>
                {{ reservation.payment }}
              </span>
            </div>
            <button class="flex h-7 w-7 items-center justify-center rounded-lg text-slate-300 transition-colors hover:bg-slate-50 hover:text-slate-500">
      ...
      <div class="rounded-2xl border border-slate-200 bg-white">
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div>
            ...
            <p class="text-xs text-slate-400">Hoy · 08:00 a 23:00</p>
          </div>
        </div>
      ...
      <div class="col-span-2 rounded-2xl border border-slate-200 bg-white">
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div>
            ...
            <p class="text-xs text-slate-400">Ocupación por día y franja horaria</p>
          </div>
        ...
        <div class="overflow-x-auto px-6 py-4 pb-6">
          <div class="min-w-[500px]">
            ...
            >
              <div class="flex items-center text-xs text-slate-400">{{ heatmapHours[rowIndex] }}</div>
              <div
      ...
      <div class="rounded-2xl border border-slate-200 bg-white">
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div>
            ...
            <p class="text-xs text-slate-400">Eventos que han ocurrido en el último día</p>
          </div>
